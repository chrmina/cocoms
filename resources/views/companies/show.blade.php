<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="space-x-2">
                @can('update', $company)
                    <a href="{{ route('companies.edit', $company) }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white hover:bg-green-700">
                        {{ __('Edit') }}
                    </a>
                @endcan
                @can('delete', $company)
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                <div class="grid grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <p class="text-gray-900">{{ $company->contact ?? 'N/A' }}</p>
                    </div>
                </div>

                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-600">Address</label>
                    <p class="text-gray-900 whitespace-pre-wrap">{{ $company->address ?? 'N/A' }}</p>
                </div>

                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Representative</label>
                        <p class="text-gray-900">{{ $company->representative ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Email</label>
                        <p class="text-gray-900">
                            @if ($company->email)
                                <a href="mailto:{{ $company->email }}" class="text-blue-600 hover:text-blue-900">{{ $company->email }}</a>
                            @else
                                N/A
                            @endif
                        </p>
                ...
                <div class="grid grid-cols-3 gap-6 mb-6">
                    <div>
                        ...
                        <p class="text-gray-900">{{ $company->telephone ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Mobile</label>
                        <p class="text-gray-900">{{ $company->mobile ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Fax</label>
                        <p class="text-gray-900">{{ $company->fax ?? 'N/A' }}</p>
                    </div>
                </div>
            ...
            @if ($company->sent_letters_count > 0)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ __('Letters Sent') }}</h3>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody>
                                @foreach ($company->sentLetters as $letter)
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        <td class="px-6 py-4 font-medium text-gray-900">{{ Str::limit($letter->subject, 50) }}</td>
                                        <td class="px-6 py-4 text-gray-600">{{ $letter->recipientCompany?->name }}</td>
            ...
            @if ($company->received_letters_count > 0)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ __('Letters Received') }}</h3>
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-100">
                                <tr>
                            ...
                            <tbody>
                                @foreach ($company->receivedLetters as $letter)
                                    <tr class="bg-white border-b hover:bg-gray-50">
                                        <td class="px-6 py-4 font-medium text-gray-900">{{ Str::limit($letter->subject, 50) }}</td>
                                        <td class="px-6 py-4 text-gray-600">{{ $letter->senderCompany?->name }}</td>
                                        <td class="px-6 py-4 text-gray-600">{{ $letter->docdate?->format('Y-m-d') ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 text-right">
                                            <a href="{{ route('letters.show', $letter) }}" class="text-blue-600 hover:text-blue-900 text-sm">View</a>
                                        </td>
