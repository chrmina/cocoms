    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                <form method="GET" action="{{ route('companies.search') }}" class="flex gap-3">
            ...
            @if ($query)
                                ...
                                <tbody>
                                    @foreach ($companies as $company)
                                        <tr class="bg-white border-b hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4 text-gray-600">{{ $company->received_letters_count }}
                                            </td>
                ...
                @else
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <p class="text-gray-500 text-center">No companies found matching "{{ $query }}"</p>
                    </div>
                @endif
            @else
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-gray-500 text-center">Enter a search term to find companies</p>
                </div>
            @endif
        </div>
    </div>
