<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="space-x-2">
                @can('update', $sender)
                    <a href="{{ route('senders.edit', $sender) }}" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white hover:bg-green-700">
                        {{ __('Edit') }}
                    </a>
                @endcan
                @can('delete', $sender)
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <p class="text-gray-900 font-semibold text-lg">{{ $sender->name }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Contact</label>
                        <p class="text-gray-900">{{ $sender->contact ?? 'N/A' }}</p>
                    </div>
                </div>

                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-600">Address</label>
                    <p class="text-gray-900 whitespace-pre-wrap">{{ $sender->address ?? 'N/A' }}</p>
                </div>

                <div class="grid grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Representative</label>
                        <p class="text-gray-900">{{ $sender->representative ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-600">Email</label>
                        <p class="text-gray-900">
                            @if ($sender->email)
                                <a href="mailto:{{ $sender->email }}" class="text-blue-600 hover:text-blue-900">{{ $sender->email }}</a>
                            @else
                                N/A
                            @endif
                        </p>
            ...
            @if ($sender->letters_count > 0)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ __('Letters from this Sender') }}</h3>
