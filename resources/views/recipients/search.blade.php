    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-6">
                <form method="GET" action="{{ route('recipients.search') }}" class="flex gap-3">
            ...
            @if ($query)
                                ...
                                <tbody>
                                    @foreach ($recipients as $recipient)
                                        <tr class="border-b border-gray-200 hover:bg-gray-50">
                                            ...
                                            <td class="px-6 py-4">{{ $recipient->representative }}</td>
                                            <td class="px-6 py-4">{{ $recipient->email }}</td>
                                            <td class="px-6 py-4">{{ $recipient->telephone }}</td>
                ...
                @else
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <p class="text-gray-500 text-center">No recipients found matching "{{ $query }}"</p>
                    </div>
                @endif
            @else
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <p class="text-gray-500 text-center">Enter a search term to find recipients</p>
                </div>
            @endif
        </div>
    </div>
